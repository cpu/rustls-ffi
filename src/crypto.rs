...
mod tests {
    ...
    fn test_ciphersuite_mix_and_match() {
        let ring_provider_builder = CryptoProviderBuilder::rustls_crypto_provider_builder_ring();
        uninitialized_mut_ptr!(ring_provider);
        let result = CryptoProviderBuilder::rustls_crypto_provider_builder_build(
            ring_provider_builder,
            ring_provider,
        );
        rustls_ok!(result, "failed to build ring crypto provider");
        assert!(!ring_provider.is_null());
