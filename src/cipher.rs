...
impl rustls_server_cert_verifier {
    ...
    pub extern "C" fn rustls_platform_server_cert_verifier(
        ...
        ffi_panic_boundary! {
            let verifier_out = try_mut_from_ptr_ptr!(verifier_out);
            let provider = match CryptoProvider::get_default().cloned() {
                Some(provider) => provider,
                None => return rustls_result::NoDefaultCryptoProvider,
            };
...
mod tests {
    ...
    use crate::crypto_provider::{
        ensure_provider, rustls_crypto_provider_ciphersuites, rustls_crypto_provider_default,
        rustls_crypto_provider_free, rustls_supported_ciphersuites_free,
    ...
    fn default_cipher_suites() {
        ...
        let ciphersuites = rustls_crypto_provider_ciphersuites(provider);
