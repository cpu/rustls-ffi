 ...
 * [`rustls-platform-verifier`][] and the process default crypto provider.
 *
...
/**
 * Create a new server certificate verifier that uses the system's root store and WebPKI via
 * [`rustls-platform-verifier`][] and the specified crypto provider.
 *
 * The verifier can be used in several `rustls_client_config` instances and must be freed by
 * the application using `rustls_server_cert_verifier_free` when no longer needed.
 *
 ...
 * then free the resulting rustls_client_config. This uses the process default provider's
 * values for the cipher suites and key exchange groups, as well as safe defaults for
 * protocol versions.
...
/**
 * Frees the `rustls_signing_key`. This is safe to call with `NULL`, or multiple times.
 */
 ...
 * then free the resulting rustls_server_config. This uses the process default provider's
 * values for the cipher suites and key exchange groups, as well as safe defaults for
 * protocol versions.
 */
 ...
 * provider reference is cloned and the caller retains ownership.
 * Caller owns the memory and must eventually call rustls_server_config_builder_build,
 * then free the resulting rustls_server_config. Set the TLS protocol
