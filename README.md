...
keeps `X25519MLKEM768` available, but at a lower negotiation priority. CMake
builds can enable this preference with `-DPREFER_POST_QUANTUM=on`; that CMake
option defaults to off.

The `prefer-post-quantum` feature only controls key-exchange priority; it does
not control ML-DSA support.
