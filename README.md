...
To change where the library is insatlled, use `--prefix` like:
...
* Building `.a`, `.so` or `.dylib` library files for rustls-ffi (depending on
  the OS).
...
cmake --build my-clang-build --target integration_test
```
