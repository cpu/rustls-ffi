      ...
      perSystem = { config, self', pkgs, pkgs-unstable, lib, system, ... }:
        let
          devDeps = with pkgs; [
            ...
            # Include some deps. that are useful when building curl w/ librustls
            autoconf
            automake
            openssl
            libtool
          ];
          # We need cargo-c 0.9.28+ to pick up the version_suffix_components
          # setting for non-semver compat.
          # TODO(XXX): switch back to stable nixpkgs once available there.
          buildDeps = [ pkgs-unstable.cargo-c ];
